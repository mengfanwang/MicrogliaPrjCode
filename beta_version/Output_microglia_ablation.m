...
file_name = 'D:\dropbox\Modify Series Data-3nd\Priority 1 files\HA-091520-slice2-hippo-microglia-Modify Series';

% function Output_microglia_ablation(file_name)
addpath ./bfmatlab

file_split = split(file_name,'\');
xls_name = ['D:\dropbox\Modify Series Data-3nd\Priority 4 files\output\' file_split{end} '.xlsx'];
...
%% for mircroglia ablation only. calculate the distance between ablation
% site to vessel
soma_ablated_all = zeros(x,y,z);
for tt = 1:t
    ind = num2str(1000+tt); 
    ind = ind(2:4);
    load([file_name '.\soma_ablated\' ind '.mat'],'soma_ablated');
    soma_ablated_all = soma_ablated_all + soma_ablated;
end
[x_ind, y_ind, z_ind] = ind2sub(size(soma_ablated), find(soma_ablated_all));
x_soma = mean(x_ind); y_soma = mean(y_ind); z_soma = mean(z_ind);
%%
ablation_center = [x_soma y_soma z_soma];
...
ablation2vessel = distance_c1(round(x_soma),round(y_soma),round(z_soma));
xlswrite(xls_name,{'Distance between ablation microglia and vessel'},'Sheet1','A18:A18');
xlswrite(xls_name,ablation2vessel,'Sheet1','B18:B18');
