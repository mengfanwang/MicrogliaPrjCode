...
file_name = 'D:\dropbox\Modify Series Data\Copy of 040319 slice1 hippo before_Modify Series';
addpath ./bfmatlab
...
location = getAblationSite(file_name);
load([file_name '\drift_c1.mat']);
location(2:2:8) = location(2:2:8) + shifts1(1).shifts(1) - x_min;
location(1:2:7) = location(1:2:7) + shifts1(1).shifts(2) - y_min;
...
xlswrite([file_name '\' xls_name],soma_num,'Sheet1','B19:B19');
%%% tracking
...
for ii = 1:length(trajectories)
    ...
    if ismember(ablation_time, t_temp)
        ...
        for jj = 1:soma_components.NumObjects
            if label_ablation(node_table(frame_ind,2),node_table(frame_ind,3),node_table(frame_ind,4)) == label_ablation(soma_components.PixelIdxList{jj}(1)) && length(soma_components.PixelIdxList{ss}) > 50
...
function location = getAblationSite(file_name)
    ...
    loc3 = strfind(strTmp, ',');
    location = zeros(1,8);
    location(1) = str2double(strTmp(1:loc3(1)-1));
    location(2) = str2double(strTmp(loc3(1)+1:loc2(1)-1)); 
    location(3) = str2double(strTmp(loc2(1)+1:loc3(2)-1));
    location(4) = str2double(strTmp(loc3(2)+1:loc2(2)-1));
    location(5) = str2double(strTmp(loc2(2)+1:loc3(3)-1));
    location(6) = str2double(strTmp(loc3(3)+1:loc2(3)-1));
    location(7) = str2double(strTmp(loc2(3)+1:loc3(4)-1));
    location(8) = str2double(strTmp(loc3(4)+1:end));
end
