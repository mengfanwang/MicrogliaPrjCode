...
mkdir([file_name 'redetection_info_iter1']);
% data_all = load([file_name 'stabilization_c2\stabilization_data.mat']);
data_all = load([file_name 'time_domain_data_after_VST\stabilization_data.mat']);
...
ind = num2str(1000+tt);
ind = ind(2:4);
foreground = load([file_name 'foreground_iter1\' ind '.mat']);
...
fprintf('Total number of objects:%d\n',com_before.NumObjects);
for ii = 1:com_before.NumObjects
%     ii
    mask_temp = zeros(x,y,z);
    mask_temp(com_before.PixelIdxList{ii}) = 1;
    dist_temp = bwdist(mask_temp);
    fprintf('%d/%d ', ii, com_before.NumObjects);
    for jj = ii+1:com_before.NumObjects
        if min(dist_temp(com_before.PixelIdxList{jj})) < d && jj~=ii
            
            [x_temp1, y_temp1, z_temp1] = ind2sub(size(foreground),com_before.PixelIdxList{ii});
            [x_temp2, y_temp2, z_temp2] = ind2sub(size(foreground),com_before.PixelIdxList{jj});
            ...
            data_temp = data(x_min:x_max, y_min:y_max, z_min:z_max);
            ...
            dist_sum = dist_sum - 1;
            dist_sum(logical(foreground(x_min:x_max, y_min:y_max, z_min:z_max))) = inf;
...
%             mask3 = (dist3 < inf) | mask3;
            mask3 = regionGrowing(data_temp, logical(foreground_temp), mask3, conn) | mask3;
            mask4 = imdilate(mask3,ones(3,3,3));
            mask4 = imdilate(mask4,ones(3,3,3)) & (~logical(foreground_temp)&~mask3&~mask4);
...
%             z_score = order_stat_test(sum(mask4(:)), sum(mask3(:)), mean(data(mask4)) - mean(data(mask3)));
            ...
            time_start = toc;
            background = data_temp(mask4);
            if length(background) > 4*sum(mask3(:))
                background = uniformSampling(background, 4*sum(mask3(:)));
            end
           
            try
                [z_score, mu, sigma] = order_stat_test_boyu(data_temp(mask3),background);
            catch
                z_score = 5;
            end
...
save([file_name 'redetection_info_iter1\t' num2str(tt) '_d' num2str(d) '.mat'],'g','fore_candidate')
% gt = load([file_name 'redetection_info\t' num2str(tt) '_d' num2str(d) '.mat'],'g','fore_candidate');
% for ii = 1:length(fore_candidate)
%     flag_temp = 0;
%     if g.Edges.EndNodes(ii,1) ~= gt.g.Edges.EndNodes(ii,1) || g.Edges.EndNodes(ii,2) ~= gt.g.Edges.EndNodes(ii,2)
%         flag_temp = 1;
%     end
%     if g.Edges.Weight(ii) ~= gt.g.Edges.Weight(ii)
%         flag_temp = 1;
%     end
%     if ~all(sort(fore_candidate{ii})==sort(gt.fore_candidate{ii}))
%         flag_temp = 1;
%     end
%     if flag_temp
%         error('result not same');
%     end
% end
end
...
mkdir([file_name '\foreground_c2_reconnect_iter1']);
for tt = 1:35
ind = num2str(1000+tt);
ind = ind(2:4);
load([file_name '\redetection_info_iter1\t' num2str(tt) '_d10.mat']);
foreground = load([file_name '\foreground_c2_iter1\' ind '.mat']);
foreground = foreground.foreground;
gaussian_sigma = 0.4;
data = data_all(:,:,:,tt);
data = imgaussfilt3(data,gaussian_sigma);
...
ind = num2str(1000+tt); 
ind = [file_name '\foreground_c2_reconnect_iter1\' ind(2:4)];
save(ind,'foreground');
